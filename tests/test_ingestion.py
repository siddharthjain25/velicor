...
def test_ingest_invalid_api_key(mock_get_service, client):
    ...
    response = client.post("/api/v1/ingest", json=payload, headers={"X-API-Key": "invalid-key"})
...
def test_single_ingest_success(mock_get_service, client):
    ...
    mock_get_service.return_value = {"name": "test-service", "user_id": "60c72b2f9b1d8e1f88a8f12a"}
    payload = {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "level": "INFO",
        "message": "Hello, this is a test log message",
    ...
    response = client.post("/api/v1/ingest", json=payload, headers={"X-API-Key": "valid-key"})
    assert response.status_code == 200

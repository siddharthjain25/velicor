...
async def send_webhook_request(url: str, logs: List[Dict[str, Any]]):
    try:
        ...
        if response.status_code >= 400:
            logger.error(
                f"Webhook delivery failed to {url}: {response.status_code} - {response.text}"
            )
        else:
...
async def send_retention_webhook_request(
    ...
    try:
                ...
                "blocks": [
                    {
                        ...
                        "text": {
                            "type": "mrkdwn",
                            "text": f"🧹 *Velicor Retention*: Purged logs for service `{service_name}`",
                        },
                    },
                    {
                        "type": "section",
                        "fields": [
                            ...
                            {
                                "type": "mrkdwn",
                                "text": f"*Logs Purged:* {deleted_count:,}",
                            },
                        ],
                    },
                ],
        ...
        if response.status_code >= 400:
            logger.error(
                f"Retention webhook delivery failed to {url}: {response.status_code} - {response.text}"
            )
        else:

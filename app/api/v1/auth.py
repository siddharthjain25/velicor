...
async def login(form_data: Annotated[OAuth2PasswordRequestForm, Depends()]):
    ...
    if user.get("two_factor_enabled"):
        ...
        return {"access_token": temp_token, "token_type": "2fa_temp", "requires_2fa": True}
...
async def read_users_me(current_user: Annotated[dict, Depends(get_current_user)]):
    ...
    current_user["two_factor_backup_codes_count"] = len(current_user.get("two_factor_backup_codes", []))
    return current_user
...
async def update_user_me(
    ...
    updated_user["two_factor_backup_codes_count"] = len(updated_user.get("two_factor_backup_codes", []))
    return updated_user
...
async def enable_2fa(
    ...
    if not secret:
        raise HTTPException(status_code=400, detail="2FA secret key has not been generated. Call setup first.")
...
async def disable_2fa(
    ...
    if not verified:
        raise HTTPException(status_code=400, detail="Invalid verification code or backup code. Please try again.")
...
async def reset_password_with_2fa(data: ResetPasswordRequest):
    ...
    if not user:
        raise HTTPException(status_code=400, detail="User not found or invalid username.")
    ...
    if not verified:
        raise HTTPException(status_code=400, detail="Invalid verification code or backup recovery code.")

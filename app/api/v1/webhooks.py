...
async def add_webhook(
    webhook: dict, current_user: Annotated[dict, Depends(get_current_user)]
):
    new_webhook = WebhookConfig(**webhook)
    await mongo_manager.db.users.update_one(
        {"_id": current_user["_id"]}, {"$push": {"webhooks": new_webhook.model_dump()}}
    )
...
async def delete_webhook(
    webhook_id: str, current_user: Annotated[dict, Depends(get_current_user)]
):
    result = await mongo_manager.db.users.update_one(
        {"_id": current_user["_id"]}, {"$pull": {"webhooks": {"id": webhook_id}}}
    )
...
async def update_webhook(
    ...
    result = await mongo_manager.db.users.update_one(
        {"_id": current_user["_id"], "webhooks.id": webhook_id}, {"$set": update_fields}
    )

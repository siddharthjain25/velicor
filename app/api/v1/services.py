    ...
    if current_user.get("two_factor_enabled"):
        ...
        if totp.verify(verification_code, valid_window=1):
            ...
            hashed_input = hashlib.sha256(
                verification_code.upper().encode("utf-8")
            ).hexdigest()
        ...
        if not verified:
            raise HTTPException(
                status_code=400, detail="Invalid 2FA verification code or backup code"
            )
...
async def reset_service_key(
    ...
    await mongo_manager.db.services.update_one(
        {"_id": obj_id}, {"$set": {"secret_key": new_secret_key}}
    )
...
async def create_service(
    service_in: ServiceCreate, current_user: Annotated[dict, Depends(get_current_user)]
):
    # Check if service name already exists for this user
    existing = await mongo_manager.db.services.find_one(
        {"user_id": str(current_user["_id"]), "name": service_in.name}
    )
    if existing:
    ...
    service_db = ServiceInDB(
        name=service_in.name, user_id=str(current_user["_id"]), secret_key=secret_key
    )
...
async def update_webhook(
    ...
    update_fields = {
        f"webhooks.$.{k}": v for k, v in webhook_update.items() if k != "id"
    }
...
async def trigger_global_purge(
    ...
    user_services = await mongo_manager.db.services.find(
        {"user_id": str(current_user["_id"])}
    ).to_list(None)
    count = 0
    from app.models.service import WebhookConfig

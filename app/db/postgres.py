...
class PostgresManager:
    ...
    async def get_connection(self):
        ...
        if settings.is_serverless:
            return await asyncpg.connect(
                settings.POSTGRES_URL, ssl="require", statement_cache_size=0
            )
    ...
    async def _create_partitioned_parent_table(
        self, table_name: str, conn: asyncpg.Connection
    ):
        await conn.execute(f"""
    ...
    async def delete_table(self, service_name: str):
        ...
        try:
            ...
            self.known_partitions = {
                k for k in self.known_partitions if not k.startswith(f"{table_name}:")
            }
    ...
    async def ensure_partition(
        self, parent_table: str, day: date, conn: asyncpg.Connection
    ):
            ...
            for row in partitions:
                ...
                if match:
                    ...
                    p_end_datetime = datetime(
                        p_year, p_month, p_day, tzinfo=timezone.utc
                    ) + timedelta(days=1)

...
async def reset_user(username, password=None, disable_2fa=False):
    ...
    if password:
        ...
        print(f"🔑 Generating secure bcrypt hash and updating password for '{username}'...")
        
    if disable_2fa:
        update_fields["two_factor_enabled"] = False
        update_fields["two_factor_secret"] = None
        update_fields["two_factor_backup_codes"] = []
        print(f"🔒 Disabling Two-Factor Authentication & clearing recovery codes for '{username}'...")
        
    if not update_fields:
        print("⚠️ No action specified. Use -p/--password to reset password or --disable-2fa to reset 2FA.")
        client.close()
        return False
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Velicor Admin Account Rescue & Password Reset Tool")
    parser.add_argument("-u", "--username", required=True, help="Username of the account to reset")
    parser.add_argument("-p", "--password", help="New plain-text password to set")
    parser.add_argument("--disable-2fa", action="store_true", help="Disable 2FA and clear recovery secrets")
